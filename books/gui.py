...
os.chdir("C:\\Users\\Mustafa\\Documents\\recommendation\\recommendation\\books")
...
# In[]
def construct_matrices(books, use_genre=True, use_summary=True, use_author=True):
    global summary_terms_begin, summary_terms_end
    Dg = []
    Ds = []
    Da = []
    ...
    for b in books:
        ...
        if b.genres is not None:
            bg = " ".join(["Genre:"+g for g in b.genres])
        Dg.append(bg)
    ...
    # Genre
    if use_genre:
        vect = CountVectorizer(token_pattern = r'\b\S+\b', lowercase=False, binary=True, dtype=np.float)
    ...
    if use_summary:
        ...
        Xp = vect.fit_transform(Ds)
        summary_terms_begin = len(vocabulary)
        vocabulary = vocabulary + vect.get_feature_names()
        summary_terms_end = len(vocabulary)
        if X is None:
            X = Xp
        else:
            X = hstack((X, Xp)).tocsr()
...
summary_terms_begin = summary_terms_end = None

X, vocabulary = construct_matrices(m_, use_summary=True, use_genre=True)
print(X.shape)

if summary_terms_begin is not None:
    for i in range(len(m_)):
        m = m_[i]
        pt = []
        pi = list(filter(lambda f: f>=summary_terms_begin and f<summary_terms_end, X[i].indices))
        for p in pi:
...
def load_a_profile(name, books):
    ...
    for i in range(len(books)):
        m = books[i]
        if m.title+m.author in training_instances:
            training_set_iids.add(i)
            m.is_in_training = True
            m.training_instance = training_instances[m.title+m.author]
        else:
...
di = (("Title", "title", False),  ("Author", "author", False), ("Date", "date", False))
...
def edit_training_data():
    ...
    def remove_selected_from_training_set():
        ...
        for iid in selected_iids:
            ...
            training_instances.pop(m.title + m.author, None)
...
def train_a_model(dataset):
    ...
    for iid in training_set_iids:
        ...
        if m.is_in_training:
            ...
            if ti.rationale_summary_terms is not None:
                for pt in ti.rationale_summary_terms:
                    try:
    ...
    di = (("Title", "title", False),  ("Author", "author", False), ("Date", "date", False), ('Score', 'pred_score', True))
...
def bulk_add_rationales():    
    ...
    summary_rationales = get_possible_rationales(training_set_iids, "summary_terms")
    _add_lb_and_button("summary_terms", rationale_window, 2, 0, summary_rationales)

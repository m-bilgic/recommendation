...
class DisplayFilterBooks(ttk.Frame):
    def __init__(self, parent, books, display_info, **kw):
        ...
        self.editorialreviewphrase = StringVar()
        ttk.Label(phrasefilterframe, text="Editorial Review").grid(column=0, row=1, sticky="W")
        ttk.Entry(phrasefilterframe, textvariable=self.editorialreviewphrase).grid(column=1, row=1, sticky="nsew")
    ...
    def _filter_data(self, *args):
        ...
        for iid in data:
            b = self.books[int(iid)]

            if b.title.find(self.titlephrase.get()) == -1:
                remove_iids.add(iid)
            
            if self.editorialreviewphrase.get() != "":
                if b.editorial_review == "":
                    remove_iids.add(iid)
                elif  b.editorial_review.find(self.editorialreviewphrase.get()) == -1:                   
                        remove_iids.add(iid)
    ...
    def _reset_filters(self, *args):
        ...
        self._update_filter_field_values()
        self.editorialreviewphrase.set("")
...
class SingleBook(ttk.Frame):
    def __init__(self, parent, b, **kw):
        super().__init__(parent, **kw)
        ttk.Label(self, text=str(b), font=font.Font(family='Helvetica', size=14, weight='bold')).grid(column=0, row=0, sticky="W")
        ...
        if b.editorial_review is not None:
            ttk.Label(info_frame, text="Editorial Review" ).grid(column=0, row=row_i, sticky="W")
            row_i += 1
            pt = Text(info_frame, height = 7, wrap='word')
            pt.insert('1.0', b.editorial_review)
            pt.grid(column=0, row=row_i, sticky="nsew")
            row_i += 1
        
        if b.editorial_review_terms is not None:            
            row_i = self._add_list_boxes(info_frame, "Editorial Review Terms", "editorial_review_terms", row_i)
